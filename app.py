...
# L'URL publique de votre service Web Render
...
# --- Fonctions de basculement, déconnexion et abonnement (corrigées) ---
...
# CORRECTION 1: Ajout de la restauration du stock du panier à la déconnexion
def logout():
    st.session_state.logged_in = False
    st.session_state.username = None
    st.session_state.is_admin = False
    ...
    # AJOUT CRITIQUE : Restaurer le stock avant de vider le panier à la déconnexion
...
# CORRECTION 2: Ajout de la réinitialisation de l'auth_mode si l'abonnement expire
    ...
    if user_info["is_active"]:
        ...
        if sub_end_date_str:
            ...
            if sub_end_date.date() < datetime.now().date():
                ...
                st.session_state.logged_in = False
                # AJOUT CRITIQUE : Réinitialiser l'état de l'authentification
                st.session_state.auth_mode = "login" 
